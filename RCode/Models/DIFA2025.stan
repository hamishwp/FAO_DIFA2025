...
transformed parameters {
  vector[n_isos] sq_sigma;
  for (iso in 1:n_isos) {
    sq_sigma[iso] = square(sigma[iso]);
  }
}
...
model {
 ...
 for(iso in 1:n_isos){
   ...
   iprox ~ normal(mu_dis[iso, 1:n_dis[iso]], sigma_dis[iso, 1:n_dis[iso]]);
   ...
   for(ttt in 1:n_t){
     ...
     for(i_dis in 1:n_dis[iso]){
       ...
       if(flag[iso,ttt,i_dis]!=0) {
         // Save on computation
         real iphs = iprox[i_dis] / hsev[htype[i_dis]]; 
         // Calculate the EOY disaster severity based on this disaster and add to total disaster severity for this EOY
         dsev += iprox[i_dis] * (duration[iso, ttt, i_dis] + iphs *(1-exp(-duration[iso, ttt, i_dis]/iphs)));
       } 
     }
