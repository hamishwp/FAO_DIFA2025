...
if(Desinventar){ # infer disaster severity from Desinventar data
  ...
  # If we want to model desinventar data not EM-DAT (significantly reducing the error in the crop loss estimate for many countries)
  GetDisSev_Des<-function(dissie){
    dissie%>%mutate(mu=crops,sd=1,dep="crops")%>%
      filter(mu>1)%>%
      mutate_at(c("duration","deaths","cost","affected","crops","cattle"), function(x) log(x+10))%>%
      GroupHazs(hazgrp_def)%>%
      # GroupHazs(hazgrps[[hazgrp_f]])%>%
      filter(!is.na(crops) & !is.na(haz_grp))%>%
      distinct()%>%mutate(disno=1:n())
  }
