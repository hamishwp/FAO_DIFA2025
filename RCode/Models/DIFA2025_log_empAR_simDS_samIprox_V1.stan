...
model {
  ...
  for(iso in 1:n_isos){
    ...
    vector[n_dis[iso]] flag_vec = rep_vector(0,n_dis[iso]);
    vector[n_dis[iso]] iproxhs = rep_vector(0,n_dis[iso]);
    vector[n_dis[iso]] hs = rep_vector(1,n_dis[iso]);
    // AR1 mean (phi)
      ...
      if(sum(flag_vec)>0){
        ...
        for(ic in 1:n_com){
          ...
          if(sum(iprox[iso, 1:n_dis[iso], ic])>0){
            ...
            dsev[ic] = log(1+sum(flag_vec.*(rep_vector(1,n_dis[iso])+to_vector(hazdur[iso, ttt, 1:n_dis[iso]])*beta_dur).*exp(iproxhs[1:n_dis[iso]])));
          }
        }
