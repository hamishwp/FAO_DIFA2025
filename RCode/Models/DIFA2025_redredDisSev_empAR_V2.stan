  ...
  for(iso in 1:n_isos){
    ...
    for(ttt in 1:n_t){
      ...
      if (ttt == 1) {
        ...
        mu = beta_dis*dsev.*(1 + isev) + beta_y1.*to_vector(mu_AR1[iso, ]).*to_vector(y[iso, ttt-1, ]); // Auto-Regressive first order (AR1) model
      }
      // Reduce the AR1 sigma values to allow disasters to account for some of the variability
      red_sig = to_vector(sig_AR1[iso,]).*sigma;
