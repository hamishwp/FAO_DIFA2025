...
parameters {
  ...
  array[n_isos,n_com] beta_y1; // GPR AR1 mean function trend, per country
...
model {
 ...
 for(iso in 1:n_isos){
   ...
   for(ttt in 1:n_t){
     ...
     if (ttt == 1) {
        mu = y[iso, ttt, ];  // No past y available
      } else {
        mu = beta_dis*dsev*(1 + csev[iso] + isev) + beta_y1[iso, ].*y[iso, ttt-1, ]; // Auto-Regressive first order (AR1) model
      }
   }
   // Sample the commodity data!
   y[iso,ttt,] ~ multi_normal_cholesky(mu, L_K);
 }
}
