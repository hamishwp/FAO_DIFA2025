...
InitParams<-function(fdf, iprox_dat=T){
  ...
  alpha_shape <- rep(NA,fdf$n_isos)
  alpha_scale <- rep(NA,fdf$n_isos)
  rho_shape <- rep(NA,fdf$n_isos)
  rho_scale <- rep(NA,fdf$n_isos)
  ...
  for (iso in 1:fdf$n_isos) {
    ...
    y <- as.data.frame.table(fdf$y[iso,,]) %>%
      ...
      alpha_shape[iso] <- gp_params$shape_alpha
      alpha_scale[iso] <- gp_params$scale_alpha
      rho_est[iso] <- gp_params$rho
      rho_shape[iso] <- gp_params$shape_rho
      rho_scale[iso] <- gp_params$scale_rho
  }
