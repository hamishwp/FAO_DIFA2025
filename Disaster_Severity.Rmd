...
subtitle: "Hamish Patten \& Ignacio Costa"
output:
...
table(dessie$haz_Ab)%>%as.data.frame()%>%
  arrange(desc(Freq))%>%
kable(col.names = c("Abbreviated Hazard","Number of Records"))
...
table(dessie$ISO3)%>%as.data.frame()%>%
  arrange(desc(Freq))%>%
kable(col.names = c("Country/Cluster ISO","Number of Records"))
